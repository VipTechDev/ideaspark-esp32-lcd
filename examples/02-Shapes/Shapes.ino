#include <IdeasparkLCD.h>

// These pins are fixed because the LCD is pre‑attached to the ESP32.
// You do NOT need to wire anything.
#define TFT_CS   15
#define TFT_DC   2
#define TFT_RST  4
#define TFT_BLK  32

IdeasparkLCD lcd(TFT_CS, TFT_DC, TFT_RST, TFT_BLK);

void setup() {
    lcd.begin();

    /*
        This example shows how to draw basic shapes using the raw()
        graphics functions from the Adafruit GFX library.

        SHAPE FUNCTIONS USED HERE:

        1) drawRect(x, y, width, height, colour)
           x, y      = top‑left corner of the rectangle
           width     = rectangle width in pixels
           height    = rectangle height in pixels
           colour    = outline colour

        2) fillCircle(x, y, radius, colour)
           x, y      = centre of the circle
           radius    = size of the circle
           colour    = fill colour

        3) drawLine(x0, y0, x1, y1, colour)
           x0, y0    = starting point
           x1, y1    = ending point
           colour    = line colour

        4) drawTriangle(x0, y0, x1, y1, x2, y2, colour)
           (x0,y0), (x1,y1), (x2,y2) = the 3 corners
           colour                    = outline colour

        Try experimenting:
        - Change colours (ST77XX_RED, ST77XX_BLUE, ST77XX_YELLOW…)
        - Move shapes around by changing x/y
        - Make shapes bigger or smaller
        - Add your own shapes below!
    */

    // Clear the screen
    lcd.fill(ST77XX_BLACK);

    // Rectangle outline
    lcd.raw().drawRect(10, 10, 80, 50, ST77XX_RED);

    // Filled circle
    lcd.raw().fillCircle(150, 40, 20, ST77XX_BLUE);

    // Line from bottom‑left to top‑right
    lcd.raw().drawLine(
        0, lcd.raw().height() - 1,
        lcd.raw().width(), 0,
        ST77XX_YELLOW
    );

    // Triangle
    lcd.raw().drawTriangle(
        60, 60,
        20, 120,
        100, 120,
        ST77XX_GREEN
    );
}

void loop() {}